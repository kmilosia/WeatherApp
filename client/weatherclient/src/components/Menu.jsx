    ...
    <div className='bg-slate-900 text-gray-100 flex flex-col p-5 h-full w-full'>
      <div className='flex w-full justify-between items-center'>
        ...
        <div className='w-max flex'>
          ...
          <button onClick={() => setManagementMode(!managementMode)} className={buttonStyle}>{managementMode ?<FaCheck size={30}/>:<BsThreeDotsVertical size={30} />}</button>
        </div>
      </div>
      {locations &&
      <ul className='my-4'>
        {locations.map((item) => {
          return(
            <MenuLocationForecast item={item} key={item} managementMode={managementMode}/>
          )
        })}
      </ul>
      }
